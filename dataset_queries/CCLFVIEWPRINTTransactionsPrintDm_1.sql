...
HAVING   Count(DISTINCT(sales_order)) =1
GROUP BY   sales_order) Header_Sales_Order ,
    ...
    a_ship_loc.country            ship_to_country,
     ...
     a_remit_loc.address1                       remit_to_address1,
       ...
       (select currency_code 
          from gl_ledger_le_v 
          where legal_entity_id = trx.legal_entity_id
              and ledger_id = primary_ledger_id
          and rownum<=1) FUNCTIONAL_CURRENCY_CODE,
...
(SELECT max(cp.email_address) 
  ...
  (select l.meaning
   ...
   and l.lookup_code = nvl(trx.finance_charges, 'N')) finance_charges,
...
to_char(nvl(ps.amount_due_original,to_number(0)) ,
        ...
        trx.comments,
        trx.START_DATE_COMMITMENT,
        trx.END_DATE_COMMITMENT,
...
,(SELECT substr(a1.address_line_3,
              1,
              instr(a1.address_line_3,'~',1,1)
              -1
              )
             FROM XLE_REGISTRATIONS_V a1
               WHERE a1.legal_entity_id = trx.legal_entity_id
			   ) LE_TEL
,(SELECT substr(a1.address_line_3,
                instr(a1.address_line_3,'~',1,1)+1,
                instr(a1.address_line_3,'~',2,2)-1
                -instr(a1.address_line_3,'~',1,1)
                )
             FROM XLE_REGISTRATIONS_V a1
               WHERE a1.legal_entity_id = trx.legal_entity_id
...
,(SELECT c1.mailstop 
   ...
   where a1.party_id = b1.party_id
    ...
    and a1.legal_entity_id = trx.legal_entity_id
    and rownum<=1	) LE_MAIL
,(select SUBSTR(e.address2,
               instr(e.address2,'PRM',1)+4,
               instr(e.address2,'~',1)
               -instr(e.address2,'PRM',1)-4)
from fusion.fnd_setid_assignments a, 
fnd_setid_sets b ,
xle_entity_profiles c,
ar_remit_to_locs_all d,
hz_locations e
where a.set_id = b.set_id
and a.reference_group_name = 'AR_REMIT_TO_ADDRESS'
and a.determinant_value = trx.org_id
and d.set_id = b.set_id
and e.location_id = d.location_id
and substr(e.address1,1,6)= c.LEGAL_ENTITY_IDENTIFIER
and c.legal_entity_id = trx.legal_entity_id
and b.language = userenv('LANG')
and rownum<= 1) local_curr_prm
,(select SUBSTR(e.address2,
               instr(e.address2,'SEC',1)+4,
               instr(e.address2,'&&')
               -instr(e.address2,'SEC')-4              
              )
from fusion.fnd_setid_assignments a, 
fnd_setid_sets b ,
xle_entity_profiles c,
...
and c.legal_entity_id = trx.legal_entity_id
and b.language = userenv('LANG')
and rownum<= 1) local_curr_sec
,(select SUBSTR(e.address2,
               instr(e.address2,'PRM',2,2)+4,
               instr(e.address2,'~',2,2)
               -(instr(e.address2,'PRM',2,2)+4)
               )
from fusion.fnd_setid_assignments a, 
fnd_setid_sets b ,
...
and c.legal_entity_id = trx.legal_entity_id
and b.language = userenv('LANG')
and rownum<= 1) fc_curr_prm
,(select SUBSTR(e.address2,
               instr(e.address2,'SEC',2,2)+4,
               length(e.address2)
               )
from fusion.fnd_setid_assignments a, 
fnd_setid_sets b ,
...
and c.legal_entity_id = trx.legal_entity_id
and b.language = userenv('LANG')
and rownum<= 1) fc_curr_sec	
,(select SUBSTR(e.address2,
               instr(e.address2,'&&',1,1)+2
               ,3
               )
from fusion.fnd_setid_assignments a, 
fnd_setid_sets b ,
...
and c.legal_entity_id = trx.legal_entity_id
and b.language = userenv('LANG')
and rownum<= 1 )  fc_curr_code
,(SELECT LISTAGG (b1.trx_number,
                ',')
                WITHIN GROUP (ORDER BY a1.apply_date)
           FROM AR_RECEIVABLE_APPLICATIONS_ALL a1, RA_CUSTOMER_TRX_ALL b1
          WHERE a1.applied_customer_trx_id = b1.customer_trx_id
            AND a1.org_id = b1.org_id
            AND a1.display='Y'
            AND a1.customer_trx_id = trx.customer_trx_id) original_inv_num
,(SELECT LISTAGG (to_char(b1.trx_date,'mm/dd/yyyy'),
                ',')
                WITHIN GROUP (ORDER BY a1.apply_date)
           FROM AR_RECEIVABLE_APPLICATIONS_ALL a1, RA_CUSTOMER_TRX_ALL b1
          WHERE a1.applied_customer_trx_id = b1.customer_trx_id
            AND a1.org_id = b1.org_id
            AND a1.display='Y'
            AND a1.customer_trx_id = trx.customer_trx_id)	original_inv_date
...
,(select LISTAGG (trim(a2.description), ',') 
                        WITHIN GROUP (ORDER BY a2.tax_rate) 
                 from  (select distinct replace(trim(b.description),CHR(10),'') description,a.tax_rate,a.trx_id 
                             FROM zx_lines_v a, zx_rates_vl b
                           WHERE a.tax_rate_id = b.tax_rate_id)	a2	
                where a2.trx_id = trx.customer_trx_id) applied_taxes
,Case when types.attribute1 = 'Y' then 
                             SUBSTR(types.name,1,6) 
                            Else '' 
                           End from_BUC
			 ,Case when types.attribute1 = 'Y' then 
                             SUBSTR(a_bill_ps.party_site_name,1,6) 
                            Else '' 
                           End to_BUC				
FROM
