...
USER_CURRENCY_CONVERSION_TYPE||'~'||CURRENCY_CONVERSION_RATE||'~'||CURRENCY_CONVERSION_DATE||'~'||JE_LINE_NUM||'~'||LINE_EFFECTIVE_DATE||'~'||CODE_COMBINATION_ID||'~'||COMPANY_CODE||'~'||ACCOUNT||'~'||TRADING_PARTNER||'~'||COST_CENTER||'~'||
  ...
  FROM (  SELECT  G.name LEDGER_NAME,
    ...
    WHERE A.je_header_id = B.je_header_id
     ...
     --AND  A.posted_date BETWEEN SYSDATE-18 AND SYSDATE
    ...
    -- AND NVL(:P_LAST_RUN_DATE,TRUNC(SYSDATE)) = NVL(:P_LAST_RUN_DATE,TRUNC(SYSDATE))
	 AND C.user_je_source_name=:p_je_source
	 AND A.posted_date BETWEEN NVL(:p_from_date,SYSDATE) AND NVL(:p_to_date,SYSDATE)
    UNION ALL
    SELECT   
                 G.name LEDGER_NAME,
                 TO_CHAR(G.ledger_id) LEDGER_ID,
