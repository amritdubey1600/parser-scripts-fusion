...
--#-----------------------------------------------------------------------------------------#
--# REL-079    Vignesh Kumar  	   04-Aug-2023		  Cirrus GED AR OTB Orders Report --# 
--#-----------------------------------------------------------------------------------------#
SELECT   order_details.bu_name,
		...
		Order_Details.Returned_Order_Flag,							
...
FROM
	...
	(SELECT /*+ MATERIALIZE */
			...
			*(DECODE(dha.transactional_currency_code,'USD',1,
			  (SELECT gdr.conversion_rate
				...
				AND TRUNC(gdr.conversion_date)= TRUNC(dha.ordered_date)
			  ...
			  ract.amount,
				...
				ract.gl_date, 
				ract.creation_date,
				ract.billing_period_start_date,
				ract.billing_period_end_date,
				...
				NVL((SELECT
						...
						AND billing_plan_id =(SELECT MAX(billing_plan_id)
												...
												WHERE dbp.fulfill_line_id=dfla.fulfill_line_id)),'One Time Billing') billing_frequency,					
	...
	FROM DOO_ORDER_ADDRESSES BILL_TO_AD,
		...
		WHERE context_code = 'GED HEADER EFF CONTEXT') dha_eff,
	...
	AND dha.status_code NOT IN 'CLOSED'
	...
	AND a.bu_name IN (SELECT meaning FROM fnd_lookup_values WHERE lookup_type ='GED_BU_NAMES' and language='US')
...
Group BY order_details.bu_name,
		 order_details.order_number,
		 order_details.line_number,
		 order_details.fulfill_line_number,
		 order_details.opportunity,
		 ...
		 bill_to.country,
		 ship_to.party_name,
		 ship_to.party_number,
		 ship_to.country,
		 bill_to.customer_type,							
		 item_details.product_subtype,
		 order_details.line_type_code, 
		 item_details.shippable_item_flag,
		 order_details.actual_ship_date,
		 order_details.fulfillment_date,
		 order_details.shipping_instructions,
		 order_details.rebook_order,	
		 order_details.rebook_reason,				   
		 order_details.original_quote,				   
		 ...
		 order_details.term_name,
		 order_details.adn,
		 order_details.segment_reporting,
		 order_details.cost_center,
		 order_details.requested_ship_date,
		 order_details.returned_order_flag
