...
AND UPPER(:P_PRINT_INDIVIDUAL)='N'
  AND exists(select 1 
                 from xla_ae_headers xah,
				      ra_cust_trx_line_gl_dist_all rctlg
			    where xah.event_id = rctlg.event_id
                  and rctlg.customer_trx_id = rct.customer_trx_id				
				  and xah.gl_transfer_date >  (SELECT MAX(ERH.processstart)
						                               FROM ess_request_history ERH,
													        ess_request_property ERP
						                              WHERE ERH.requestid = ERP.requestid
													    AND ERH.definition ='JobDefinition://oracle/apps/ess/custom/ar/CCLF_CANADA_BULK_INVOICE'
						                                AND ERH.executable_status = 'SUCCEEDED'
														AND ERP.name = 'submit.argument2'
...
WHERE rct.cust_trx_type_seq_id=rcta.cust_trx_type_seq_id
  ...
  AND cc.company_code NOT IN (SELECT b.flex_value company_code  
                                 ...
                                 WHERE a.flex_value_set_id=b.flex_value_set_id
                                   AND a.flex_value_set_name='CCL_CA_INVOICE_NOPRINT')
AND UPPER(:P_PRINT_INDIVIDUAL)='Y'
  AND exists(select 1 
               from xla_ae_headers xah,
				    ra_cust_trx_line_gl_dist_all rctlg
			  where xah.event_id = rctlg.event_id
                and rctlg.customer_trx_id = rct.customer_trx_id				
				and trunc(xah.gl_transfer_date) != trunc(sysdate)
                and xah.gl_transfer_date is not null)
UNION
SELECT  'NO_DATA_FOUND' trx_number,
             NULL,
             NULL ,
             NULL,
             NULL Invoice_date,
