...
--# --------------------------------------------------------------------------------------------------#
--#  REL 02    Ajaish Antony    28-FEB-2017     REl #02. Case # 15517963. Added condition 
--#                                                                                 to fix the duplicate records
--# 	
--# REL 09    Chandrakanth Makam	21-OCT-2017	REl #09. Case # 15736118 . Added subquery to get correct Deprecation_method value
--#                                                                              
--# --------------------------------------------------------------------------------------------------#
...
FADD.current_units),2)) NET_BOOK_VALUE,*/
                ...
                --FM.METHOD_CODE DEPRICIATION_METHOD,  Commented by chandrakanth for #REl 009 and #Case : 15736118
				(SELECT mth.method_code FROM FA_METHODS mth WHERE mth.method_id=a.method_id) depriciation_method, --added by chandrakanth For #REL 009 and #Case : 15736118
...
,(SELECT case 
          when amortization_start_date is not null
             then 'Y'
                                  else null
                   end amortize_flag
...
and asset_id = FADD.asset_id
    AND ROWNUM <=1) amortize_flag,
(SELECT TO_CHAR(amortization_start_date ,'YYYY/MM/DD')
   FROM FA_TRANSACTION_HEADERS 
   WHERE BOOK_TYPE_CODE = a.book_type_code
    AND TRANSACTION_SUBTYPE = 'AMORTIZED'
    and asset_id = FADD.asset_id
    AND ROWNUM <=1) amortization_start_date              
          ...
          WHERE
            ...
            AND a.book_type_code = f.book_type_code        -- Added REL 02
