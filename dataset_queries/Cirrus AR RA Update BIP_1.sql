...
--# REL-058                     Sai Sree Sowmya         12-NOV-2021       To exclude ORA_Subscriptions from GL Date update --##
--#-----------------------------------------------------------------------------------------------------#
...
DECLARE
  ...
  SELECT rila.* 
    ...
    --REL-019 EMG  added above code  	
	 ...
	 AND bu.organization_name IN (SELECT lookup_code FROM fnd_lookup_values WHERE lookup_type='CIRRUS_AUTO_INV_BU_NAMES' AND language='US' AND enabled_flag='Y') -- REL-017 added 
	 ...
	 AND rila.batch_source_name<>'ORA_Subscriptions'   --REL-058 Added
 ...
 SELECT rila.* 
	 ...
	 AND gl.name=NVL(:LEDGER_NAME,gl.name)
	 AND bu.organization_name IN (SELECT lookup_code FROM FND_LOOKUP_VALUES WHERE lookup_type='CIRRUS_AUTO_INV_BU_NAMES' AND language='US' AND enabled_flag='Y')
	 AND bu.organization_name=NVL(:ORG_NAME,bu.organization_name)
	 AND rila.batch_source_name=NVL(:TRANSACTION_SOURCE,rila.batch_source_name)
	 AND rila.batch_source_name<>'ORA_Subscriptions' --REL-058 Added
	 AND rila.interface_line_attribute1=NVL(:SALES_ORDER_NUM,rila.interface_line_attribute1);
