...
FROM
  ...
  /*AND   ((ccv.functional_currency  = 'USD' AND l.name LIKE '%FCY') OR   -- Non US Functional
         (ccv.functional_currency <> 'USD' AND l.name LIKE '%PRM' AND l.name <> 'US_USD_PRM') OR  -- Non US Primary
         (ccv.functional_currency <> 'USD' AND l.name LIKE '%FCY' AND SUBSTR(l.name,3,3) = ccv.functional_currency ) OR  -- Non US Primary         
         (l.name LIKE 'US%FCY%' AND SUBSTR(l.name,3,3) = ccv.functional_currency )  -- US FCY
         (ccv.functional_currency  = 'USD' AND l.name ='US_USD_PRM')  -- US Primary
        )*/
   AND ( 
