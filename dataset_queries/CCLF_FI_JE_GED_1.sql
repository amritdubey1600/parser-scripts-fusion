...
LEDGER_NAME||'~'||LEDGER_ID||'~'||LEDGER_SHORT_NAME||'~'||LEDGER_CATEGORY_CODE||'~'||LEDGER_CURRENCY_CODE||'~'||BATCH_ID||'~'||BATCH_NAME||'~'||BATCH_DESCRIPTION||'~'||JOURNAL_ID||'~'||JOURNAL_NAME||'~'||JOURNAL_DESCRIPTION||'~'||USER_JE_SOURCE_NAME||'~'||
USER_JE_CATEGORY_NAME||'~'||DOCUMENT_SEQUENCE_ID||'~'||DOCUMENT_SEQUENCE_VALUE||'~'||CLOSE_ACCT_SEQ_ASSIGN_ID||'~'||CLOSE_ACCT_SEQ_VALUE||'~'||PERIOD_NAME||'~'||STAT_PERIOD||'~'||EFFECTIVE_DATE||'~'||POSTED_BY||'~'||POSTED_DATE||'~'||STATUS||'~'||CURRENCY_CODE||'~'||
USER_CURRENCY_CONVERSION_TYPE||'~'||CURRENCY_CONVERSION_RATE||'~'||CURRENCY_CONVERSION_DATE||'~'||JE_LINE_NUM||'~'||LINE_EFFECTIVE_DATE||'~'||CODE_COMBINATION_ID||'~'||COMPANY_CODE||'~'||ACCOUNT||'~'||TRADING_PARTNER||'~'||COST_CENTER||'~'||
  GEOGRAPHY||'~'||PROJECT_CODE||'~'||REFERENCE_CODE||'~'||PRODUCT_LINE||'~'||BOOK_TYPE||'~'||FUTURE1||'~'||FUTURE2||'~'||ENTERED_DEBIT||'~'||ENTERED_CREDIT||'~'||ACCOUNTED_DEBIT||'~'||ACCOUNTED_CREDIT||'~'||ENTERED_AMOUNT||'~'||ACCOUNTED_AMOUNT||'~'||LINE_DESCRIPTION||'~'||
  DXL_FILE_NAME||'~'||ACCRUAL_REVERSAL_STATUS||'~'||REVERSAL_PERIOD||'~'||REVERSAL_FLAG||'~'||ACCRUAL_REV_JE_HEADER_ID||'~'||PARENT_JE_HEADER_ID||'~'||REVERSED_JE_HEADER_ID||'~'||CONVERSION_FLAG||'~'||APPROVAL_STATUS_CODE||'~'||APPROVAL_STATUS_MEANING||'~'||
  APPROVER_EMPLOYEE_ID||'~'||GROUP_ID||'~'||STAT_CURRENCY_CONV_DATE||'~'||CONTEXT||'~'||ATTRIBUTE1||'~'||ATTRIBUTE2||'~'||ATTRIBUTE3||'~'||ATTRIBUTE4||'~'||ATTRIBUTE5||'~'||ATTRIBUTE6||'~'||ATTRIBUTE7||'~'||ATTRIBUTE8||'~'||ATTRIBUTE9||'~'||ATTRIBUTE10||'~'||
  ATTRIBUTE11||'~'||ATTRIBUTE12||'~'||ATTRIBUTE13||'~'||ATTRIBUTE14||'~'||ATTRIBUTE15||'~'||ATTRIBUTE16||'~'||ATTRIBUTE17||'~'||ATTRIBUTE18||'~'||ATTRIBUTE19||'~'||ATTRIBUTE20||'~'||REFERENCE_1||'~'||REFERENCE_2||'~'||REFERENCE_3||'~'||REFERENCE_4||'~'||
  REFERENCE_5||'~'||REFERENCE_6||'~'||REFERENCE_7||'~'||REFERENCE_8||'~'||REFERENCE_9||'~'||REFERENCE_10||'~'||CREATED_BY||'~'||CREATION_DATE||'~'||LAST_UPDATED_BY||'~'||LAST_UPDATE_DATE||'~'||SUBMITTED_FOR_VALIDATION_BY||'~'||SUBMITTED_FOR_VALIDATION_DATE 
  FI_JE_ENTRY
...
--     AND  ((fvv.attribute5 = 'USD' AND g.name LIKE '%FCY') OR  (fvv.attribute5 <> 'USD' AND g.name LIKE '%PRM'))      
   AND ( 
     ...
     AND ((:P_FROM_DATE IS NOT NULL AND :P_TO_DATE IS NOT NULL AND
            A.posted_date BETWEEN :P_FROM_DATE AND :P_TO_DATE) OR
          (:P_FROM_DATE IS NULL AND :P_TO_DATE IS NULL AND
            A.posted_date > SYSDATE-10))
    UNION ALL
    SELECT   
                 G.name LEDGER_NAME,
                 TO_CHAR(G.ledger_id) LEDGER_ID,
