...
--#-----------------------------------------------------------------------------------------------------#
--# REL-046   			Sowndarya Perumal  	  02-NOV-2020  		  Credit Memo Query - Initial Version --#
--#
--#-----------------------------------------------------------------------------------------------------#
SELECT
    ract.trx_number,
...
.fn_get_line_description(ractl.customer_trx_line_id) ), (
        ...
        WHERE
            TO_CHAR(fulfill_line_id) = ractl.interface_line_attribute5
...
(ractl.customer_trx_line_id),NULL,ar_bpa_utils_pkg.fn_get_line_description(ractl.customer_trx_line_id) ), (
    ...
    NVL( (
        ...
        FROM
            RA_CUSTOMER_TRX_LINES_ALL lines
        WHERE
            lines.customer_trx_id = ract.customer_trx_id
            AND lines.line_type = 'LINE'
            AND UPPER(description) NOT LIKE '%WURLDTECH%TAX%LINE%'
            AND UPPER(description) NOT LIKE '%WT%GST/HST%TAX%'
            AND UPPER(description) NOT LIKE '%WT%PST%TAX%'
    ),TO_NUMBER(0) ) CREDIT_LINE_AMOUNT,
    NVL( (
        SELECT
            SUM(tax_amt)
        FROM
            ZX_LINES_V zlv
        WHERE
            zlv.trx_id = ract.customer_trx_id
    ...
    NVL( (
        ...
        WHERE
            ...
            AND UPPER(description) NOT LIKE '%WT%PST%TAX%'
                ...
                ---Rel - 009 added start below by CG team ID 382
     + NVL( (
        SELECT
            SUM(extended_amount)
        FROM
     ...
     + NVL( (
        ...
        WHERE
            zlv.trx_id = ract.customer_trx_id
            AND zlv.APPLICATION_ID = 222 -- In some Invoices/CM application ID is NULL so hard coding to 222 which is for Receivables Added by MEHUL
    ),TO_NUMBER(0) ) CREDIT_TOTAL_AMOUNT
FROM
...
WHERE
    ract.customer_trx_id = araa.customer_trx_id
