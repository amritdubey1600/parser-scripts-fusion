...
--#-----------------------------------------------------------------------------------------------------#
SELECT 'I10' record_key_i10,
			   operating_unit,
			   party_name,
			   account_number,
              (select substr(dbms_random.value,-6) from dual) batch_header_id,
              RPAD(trx_number,30) invoice_number,
			  RPAD((CASE 
			       WHEN ((PURCHASE_ORDER IS NOT NULL) AND
			            (DECODE(xy.source,'CONTRACT INVOICES', 
										  (SELECT DISTINCT c.attribute8 
											from  OKC_K_HEADERS_ALL_B C,
												 ...
												 where A.project_id= b.Project_id
												  ...
												  and rct.customer_trx_id= xy.customer_trx_id
												  and rownum<2
										  ...
										  (SELECT dlfeb.attribute_char2
											 ...
											 WHERE a.header_id=b.header_id
											  ...
											  -- AND rctla.INTERFACE_LINE_ATTRIBUTE3= a.ORDER_NUMBER
											   ...
											   AND rownum<2
											   AND xy.customer_trx_id = rctla.customer_trx_id
											  ...
											  -- AND RCTLA.INTERFACE_LINE_ATTRIBUTE2 =a.SOURCE_ORDER_SYSTEM
											   ...
											   NVL((SELECT distinct ral.attribute7 FROM RA_CUSTOMER_TRX_LINES_ALL ral WHERE ral.customer_trx_id = xy.customer_trx_id AND ral.attribute7 IS NOT NULL AND ROWNUM<2  AND ral.line_type = 'LINE')
												 , case when (xy.source = 'Oracle DOO Integrated Subscriptions'					--REL-ARR Added for Subscription enablement - Start
															and exists (SELECT 1 FROM HR_OPERATING_UNITS hou, FND_LOOKUP_VALUES_VL flv, RA_CUSTOMER_TRX_LINES_ALL rctla
																		WHERE rctla.customer_trx_id = xy.customer_trx_id
														...
														then (SELECT distinct dfleb.ATTRIBUTE_CHAR2
																...
																WHERE xy.customer_trx_id           = rctla.customer_trx_id
														...
														when (xy.source = 'GEV ES Intercompany'							--REL-ARR Added for Intercompany enablement - Start
															and exists (SELECT 1 FROM HR_OPERATING_UNITS hou, FND_LOOKUP_VALUES_VL flv, RA_CUSTOMER_TRX_LINES_ALL rctla
																		WHERE rctla.customer_trx_id = xy.customer_trx_id
																		AND hou.organization_id = rctla.org_id 
																		AND rctla.line_type = 'LINE'
														...
														then (SELECT distinct dfleb2.ATTRIBUTE_CHAR5
																...
																AND TRUNC(sysdate) between TRUNC(nvl(flv.start_date_active,sysdate)) AND TRUNC(nvl(flv.end_date_active,sysdate))
																AND ROWNUM                <2
																AND rctla.line_type         = 'LINE') end )					--REL-ARR Added for Intercompany enablement - End
																))IS NOT NULL)  THEN ' '
				    ELSE PURCHASE_ORDER
                END),30) 	po_number,		--R04
			  --RPAD(DECODE(purchase_order,NULL,' ',purchase_order),30) po_number,
              RPAD(' ',1) not_used_1,
              RPAD(UPPER(to_buc),6) receiver_buc,
              RPAD(invoice_currency_code,3) currency_code,
              RPAD(' ',4) not_used_2,
              RPAD(NVL(unallowable,'N'),1) unallowable,
              RPAD(NVL(profit,'N'),1) profit,
              bill_to_site_use_id,
              ship_to_site_use_id,
              customer_trx_id,
              invoice_currency_code,
              invoice_currency_precision inv_curr_precision, 
              functional_currency_code,
              buc_currency_code,
              NVL(buc_currency_precision,functional_currency_precision) buc_func_curr_precision,
              NVL(buc_cur_exchange_rate,exchange_rate) conversion_rate ,
              NVL(exchange_date,trx_date) exchange_date,
              from_BUC,
              tax_adn
         FROM (SELECT DISTINCT  
      c.segment1 company_code, a.trx_number,a.purchase_order,a.customer_trx_id, a.operating_unit, a.bill_to_site_use_id, a.ship_to_site_use_id,
       a.invoice_currency_code, d.precision invoice_currency_precision, a.account_number, a.party_name,
       a.functional_currency_code, a.functional_currency_precision, a.exchange_rate,
       a.exchange_date, a.trx_date,a.attribute2 unallowable, a.attribute3 profit, a.attribute9 override_from_buc,
        SYSDATE creation_date,
      nvl(a.meaning_new, SUBSTR(a.transaction_type,1,6)) from_BUC, a.to_BUC, tax_adn,
    ----   a.purchase_order,
   	a.buc_currency_code,
   	(CASE WHEN ((a.functional_currency_code <> NVL(a.buc_currency_code,'###')) AND (a.invoice_currency_code = a.buc_currency_code)) THEN 1
	...
	END) buc_cur_exchange_rate,
   	(CASE WHEN ((a.functional_currency_code <> NVL(a.buc_currency_code,'###')) AND (a.invoice_currency_code = a.buc_currency_code)) THEN ( SELECT precision
	                                    											     FROM FND_CURRENCIES 
	                                    											     WHERE currency_code = a.buc_currency_code )
	         WHEN (a.functional_currency_code <> NVL(a.buc_currency_code,'###')) THEN ( 	SELECT d.precision
	                 							     	FROM GL_DAILY_CONVERSION_TYPES b,
											      GL_DAILY_RATES c,
											      FND_CURRENCIES d
											WHERE b.conversion_type = c.conversion_type
	...
	a.source
     FROM ( SELECT       a.customer_trx_id,
			...
			AND hzca.cust_acct_site_id = hcs.cust_acct_site_id
			AND hcs.site_use_code = 'BILL_TO'
			AND hcs.site_use_id = a.bill_to_site_use_id) tax_adn,	
   ...
   -- AND (a.purchase_order IS NOT NULL OR NOT EXISTS(SELECT '1' FROM RA_CUSTOMER_TRX_LINES_ALL ral WHERE ral.customer_trx_id = a.customer_trx_id AND ral.attribute7 IS NULL AND ral.line_type = 'LINE'))
	AND ((a.operating_unit='CA_CAD_BU') OR ((A.OPERATING_UNIT!= 'CA_CAD_BU') AND (a.purchase_order IS NOT NULL 
