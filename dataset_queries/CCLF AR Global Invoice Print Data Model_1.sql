...
--# CREATED BY    : Shankar U                                                                          -#
--# CR#                       Author             Date                Description                       -#
--#-----------------------------------------------------------------------------------------------------#
--# REL-023                  Karun           03-DEC-2018             Added logic to Decode       -#
--#                                                                  Old LE Names with New LE names    -#
--# REL-041					 Bhanu Movva      22-MAY-2020           LE Rebranding Logo and LE name change code for PHILLIPINES and INDONESIA-#
--# REL-041                  Sindhura Puppala 26-MAY-2020           Added ReasonCode and BusinessUnitName -#
--# REL-082                  Maheswari Kali   10-OCT-2023           Added logic to change the LE name -#
--# REL-103                  Amjad M           10-AUG-2025           GERITM50197867:change logic as legal entity name changed for DGT2-#
--#-----------------------------------------------------------------------------------------------------#

SELECT xep.LEGAL_ENTITY_IDENTIFIER, -- Added for REL-082 GE II LE Name Change
 ...
 -- REL-023 commented out -- xep.name legal_entity ,
 -- REL-023 added below code
...
and flv.language='US'
and rct1.customer_trx_id=rct.customer_trx_id  --REL-103 added
...
AND rct1.legal_entity_id=:p_legal_entity
AND rct1.org_id=:p_business_unit
and rct1.customer_trx_id=rct.customer_trx_id  --amjad
and flv.language='US'
and trunc(rct.trx_date)>= trunc(flv.START_DATE_ACTIVE)  --Added for REL-082 GE II LE Name Change end
), NVL (
          (SELECT description
          ...
          xep.name))
          AS legal_entity, 
    -- REL-023 added above code  
   -- REL-023  commented out below code
/*  (SELECT a1.registration_number
  FROM XLE_REGISTRATIONS_V a1
  WHERE a1.legal_entity_id = xep.legal_entity_id
  ) LE_REGN_NO ,
     -- REL-023  commented out above code */
       -- REL-023 added below code
  		   NVL (
          (SELECT description
             FROM FND_LOOKUP_VALUES_VL
            WHERE     lookup_type = 'CIRRUS_AR_NEW_LE_DETAILS'
                  AND meaning = 'LE_REGN_NO_' || hou.name
                  AND enabled_flag = 'Y'
                  AND ROWNUM = 1
                  AND TRUNC (SYSDATE) BETWEEN NVL (start_date_active,
                                                   TRUNC (SYSDATE))
										 ...
										 (SELECT a1.registration_number
										  FROM XLE_REGISTRATIONS_V a1
										  WHERE a1.legal_entity_id = xep.legal_entity_id
										   and a1.IDENTIFYING(+)='Y'  --REL-103 added
										  ))  AS le_regn_no,
	   -- REL-023 added above code 									  
  ...
  WHERE zp.party_id =hps.party_site_id
  AND rownum       <=1
  ) rep_registration_number, --Added by Sandhya for PH print
  --  Added Below code by Bhanu for LE rebranding ID and PH --REL041
 CASE 
	WHEN rct.trx_date < (SELECT TO_DATE(meaning,'mm/dd/rrrr') FROM FND_LOOKUP_VALUES_VL 
													WHERE lookup_type = 'CIRRUSAR_OLD_LE_DETAILS' 
													AND lookup_code = xep.LEGAL_ENTITY_IDENTIFIER)
    THEN xep.name
    WHEN rct.trx_date >= (SELECT TO_DATE(meaning,'mm/dd/rrrr') FROM FND_LOOKUP_VALUES_VL 
													WHERE lookup_type = 'CIRRUSAR_OLD_LE_DETAILS' 
													AND lookup_code = xep.LEGAL_ENTITY_IDENTIFIER)
	THEN (SELECT description from FND_LOOKUP_VALUES_VL WHERE lookup_type= 'CIRRUSAR_OLD_LE_DETAILS' AND lookup_code = xep.LEGAL_ENTITY_IDENTIFIER)
	ELSE 'GE IMAGE'
  END LE_NEW_OLD
--  Added above code by Bhanu for LE rebranding ID and PH --REL041
--REL-041 Added below code for Singapore Credit Note by Sindhura Puppala  
  ,rct.reason_code
  ,hou.name business_unit
--REL-041 Added above code for Singapore Credit Note by Sindhura Puppala
FROM ra_customer_trx_all rct ,
  ...
  hz_locations hl1 ,
  fnd_currencies fc
  ,HR_OPERATING_UNITS hou -- REL-023 added
...
AND hou.organization_id             = hoi.organization_id  -- REL-023 added
