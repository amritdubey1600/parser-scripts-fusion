...
attribute19||','||attribute20 DR4_AVIMIL
FROM
(SELECT   'CCLFGL'          sender_id,
    ...
    WHERE gjh.je_header_id             = gjl.je_header_id
     ...
     AND  gjh.posted_date >= 
           ...
           WHERE  ERH.definition = 'JobDefinition://oracle/apps/ess/custom/cclf/CCLF_CA_DR4_AVIMIL'
           AND    ERH.executable_status = 'SUCCEEDED'
			) ,gjh.posted_date)
     AND  (NVL(entered_dr,0) - NVL(entered_cr,0) <> 0 OR NVL(accounted_dr,0) - NVL(accounted_cr,0) <>0)
     --AND  segment1 IN('V068', 'V069', 'V070', 'V071', 'V072', 'V075')
     AND  segment1 IN(SELECT distinct  b1.FLEX_VALUE
FROM fnd_flex_value_sets a1,
     fnd_flex_values b1,
     fnd_flex_values_tl c1
WHERE a1.flex_value_set_id   =  b1.flex_value_set_id
  AND a1.flex_value_set_name = 'CCLF_DR4_AVIATION_MIL_COMP' 
  AND b1.value_category      = 'CCLF_DR4_AVIATION_MIL_COMP'
  AND b1.flex_value_id       = c1.flex_value_id
  AND c1.language            = userenv('LANG')
  AND b1.ENABLED_FLAG        = 'Y')	 
	 ))
SELECT 'CCLFGL' KEY,CCLFUS_DR4_AVIMIL.DR4_AVIMIL DR4_AVIMIL
FROM CCLFUS_DR4_AVIMIL
UNION
SELECT 'CCLFGL' KEY,'CCLFGL,CCLJEEXIES,'||TO_CHAR(SYSDATE,'YYYYMMDD')||',TRAILER,'||TO_CHAR(COUNT(*)) ||',P' DR4_AVIMIL FROM CCLFUS_DR4_AVIMIL
ORDER BY 2 desc