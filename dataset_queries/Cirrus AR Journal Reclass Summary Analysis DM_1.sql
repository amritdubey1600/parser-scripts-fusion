  ...
  FROM
    (
        ...
        WHERE
			...
			AND TRUNC(rec_line.accounting_date) BETWEEN (TO_DATE(:p_from_date,'YYYY-MM-DD'))
        ...
        SELECT
            1 AS "KEY",
            'Receipt' transaction_type--rec_line.transaction_type
           ,
            rec_line.ledger_name,
            rec_line.bu_name,
...
AND TRUNC(araa.gl_date) BETWEEN (TO_DATE(:p_from_date,'YYYY-MM-DD'))
							 AND NVL(TO_DATE(:p_to_date,'YYYY-MM-DD'), TRUNC(SYSDATE))
        UNION ALL
        SELECT
            1 AS "KEY",
...
--Adjustment start
        ...
        SELECT
           ...
           ,
            ...
            rec_line.segment4 segment4  --Segment4
           ,
        ...
        WHERE
            ...
            AND glp.period_set_name = 'CCL CALENDAR'
            AND glp.period_name NOT LIKE 'ADJ%'
			AND TRUNC(aaa.gl_date) BETWEEN (TO_DATE(:p_from_date,'YYYY-MM-DD'))
							 AND NVL(TO_DATE(:p_to_date,'YYYY-MM-DD'), TRUNC(SYSDATE))
        UNION ALL
        SELECT
            1 AS "KEY",
        ...
        WHERE
            ...
            AND aaa.customer_trx_id = rec_line.customer_trx_id
            AND aaa.status = 'A'
			AND aaa.posting_control_id 			   != 			-3 --for not pikcing unposted
			AND aaa.type						   !=			'TAX'
            AND aaa.gl_date BETWEEN glp.start_date AND glp.end_date
            AND glp.period_set_name = 'CCL CALENDAR'
            AND glp.period_name NOT LIKE 'ADJ%'
