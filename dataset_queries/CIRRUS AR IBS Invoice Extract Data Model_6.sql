SELECT 'I29' record_key_i29,
      ...
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC'  and LATEST_REC_FLAG  ='Y'  ) invoice_amount_sign,
      (SELECT LPAD(ABS(SUM(ROUND(NVL(amount,0),fc.precision)) * POWER(10,fc.precision)),15,0) FROM RA_CUST_TRX_LINE_GL_DIST_ALL rct_in 
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and LATEST_REC_FLAG  ='Y' ) invoice_amount, -- REL-036
      ...
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and nvl(LATEST_REC_FLAG,'Y') ='Y' ) book_amount,  
      ...
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and nvl(LATEST_REC_FLAG,'Y') ='Y') book_amount_sign,
      (SELECT DECODE(gl.currency_code, c.attribute2, (SUM(NVL(acctd_amount,1)) * POWER(10,fc.precision))
      , (SUM(ROUND((amount *  nvl(rct.exchange_rate,1)), fc.precision)) * POWER(10,fc.precision))
      ) FROM RA_CUST_TRX_LINE_GL_DIST_ALL rct_in 
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and nvl(LATEST_REC_FLAG,'Y') ='Y') acctd_amount_actual
	,SUBSTR((SELECT LPAD(ABS(SUM(ROUND(NVL(amount,0),fc.precision)) * POWER(10,fc.precision)),15,0) FROM RA_CUST_TRX_LINE_GL_DIST_ALL rct_in 
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and nvl(LATEST_REC_FLAG,'Y') ='Y' ),15,1) invoice_amount_last_char
      ...
      WHERE rct_in.customer_trx_id = rctg.customer_trx_id AND account_class = 'REC' and nvl(LATEST_REC_FLAG,'Y') ='Y' ),15,1) book_amount_last_char,
	rct.customer_trx_id
