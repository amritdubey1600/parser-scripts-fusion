...
--# REL-038 EMG			Amjad/Om		   18-MAR-2020			Code changes for  GEINC6409009 REL-038 EMG to match count and avoid duplicates  
--# REL-045             Sowndarya          12-OCT-2020          Added logic to send Non IBS internal customer invoices to GECARS for collection       
--# REL-053             Om                 12-JUN-2021          GERITM19483699 Added logic to send receipt number in PONUMBER for Receipts
--# REL-069				Akash              04-OCT-2022          Added logic to include Invoices with India E invoice context
--# ---------------------------------------------------------------------------------------------------------------------
*/
...
(SELECT DISTINCT
  ...
  FROM RA_CUSTOMER_TRX_ALL RCT,
       ...
       hz_party_site_uses hpsu, ---removed comment added ship to join for GEINC6409009 REL-038 EMG
...
WHERE     HCA.CUST_ACCOUNT_ID = RCT.BILL_TO_CUSTOMER_ID
       ...
       --AND HCA.CUSTOMER_TYPE = 'R' --REL-045 Commented line 
	   --REL-045 Added below code
	   AND (HCA.CUSTOMER_TYPE = 'R' 
						OR (hca.cust_account_id IN (SELECT lookup_code 
                                                     FROM FND_LOOKUP_VALUES 
                                                    WHERE lookup_type='CIRRUSGSAR_GECARS_CUS_INV'
                                                      AND language='US'
                                                      AND TRUNC (SYSDATE) BETWEEN NVL (start_date_active, TRUNC (SYSDATE)) AND NVL (end_date_active, TRUNC (SYSDATE))
                                                      AND enabled_flag = 'Y' ) 
							AND rct.org_id IN (SELECT bu_id 
     ...
     -- Start of changes commented below lines and added ship to join for GEINC6409009 REL-038 EMG 
       --AND HCA1.CUST_ACCOUNT_ID = RCT.BILL_TO_CUSTOMER_ID
       AND HCA1.PARTY_ID = RCT.SHIP_TO_PARTY_ID --added omp
     -- End of changes commented below lines and added ship to join for GEINC6409009 REL-038 EMG 			  
       ...
       AND hpsu.site_use_type      = 'SHIP_TO'    ---  removed comment as per required changes for GEINC6409009 REL-038 EMG
       AND hpsu.party_site_use_id  =rct.ship_to_party_site_use_id -- removed comment as per required changes for GEINC6409009 REL-038 EMG
       AND hps1.party_site_id      = hpsu.party_site_id  --removed comment as per required changes for GEINC6409009 REL-038 EMG
       ...
       AND hcsu.site_use_id=rct.bill_to_site_use_id  --- removed comment as per required changes for GEINC6409009 REL-038 EMG
    ...
    --   AND HCA1.CUSTOMER_TYPE = 'R'                         --NEWLY ADDED comment as per required changes for GEINC6409009 REL-038 EMG 
       AND HCAS1.STATUS = 'A'                                    --NEWLY ADDED
       AND NVL (RCT.ATTRIBUTE1, 'Y') != 'N'
       AND APS.ACCTD_AMOUNT_DUE_REMAINING <> 0
  --     and  hcsu.PRIMARY_FLAG = 'Y'  --- COMMENTED as per required changes for GEINC6409009 REL-038 EMG
  --     AND HCSU1.PRIMARY_FLAG = 'Y'  --- COMMENTED as per required changes for GEINC6409009 REL-038 EMG
...
End InvoiceTransAmount,
  ...
  NULL InvoiceReqNO,
  NULL InvoiceBillLading,
  --REL-053 GERITM19483699 code change start here	
  -- NULL InvoicePONumber,
  RCT.RECEIPT_NUMBER InvoicePONumber,
  --REL-053 GERITM19483699 code change end here  
  NULL InvoiceNotifyDate,
  NULL InvoiceCIEDate,
  NULL InvoiceOrderSORID,
  '1' InvoiceItemNO,
...
WHERE     HCA.CUST_ACCOUNT_ID = RCT.BILL_TO_CUSTOMER_ID
	   --REL-045 Added below code
	   AND (NVL(RCT.ATTRIBUTE_CATEGORY,'X') =nvl(DECODE (RCT.ATTRIBUTE_CATEGORY,'CIRRUSGSAR_NOTES','CIRRUSGSAR_NOTES',RCT.ATTRIBUTE_CATEGORY),'X')
		--	AND NVL(RCT.ATTRIBUTE7,'1') = DECODE (RCT.ATTRIBUTE7,'No','Dont print','Yes','Yes','1')  ) -- Commented for REL-069
	   AND NVL(RCT.ATTRIBUTE7,'1') = DECODE (RCT.ATTRIBUTE7,'No','Dont print','Yes','Yes','1')   OR (rct.attribute_category = 'India E invoice')) -- Added for REL-069
	   --REL-045 Added above code
       AND RCTLG.EVENT_ID = XAH.EVENT_ID
       AND C.BU_ID = RCT.ORG_ID
       --AND  GCC.segment1         IN ('V833', 'V834')
...
--AND hCA.customer_type            = 'R' --REL-045 Commented Line
--REL-045 Added below code
AND (hca.customer_type = 'R' OR ((hca.customer_type,rct.org_id) IN (SELECT a.customer_type,ract.org_id 
