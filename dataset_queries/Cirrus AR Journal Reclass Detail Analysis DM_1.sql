...
--#-----------------------------------------------------------------------------------------------------#
--# REL-043 				Sowndarya Perumal  10-Aug-2020  		Initial version								  --#                                                                                 
--# 
--#-----------------------------------------------------------------------------------------------------#
  ...
  FROM
    (
        ...
        WHERE
            ...
            AND TRUNC(rec_line.accounting_date) BETWEEN ( TO_DATE(:p_from_date,'YYYY-MM-DD') ) AND NVL(TO_DATE(:p_to_date,'YYYY-MM-DD'),TRUNC(SYSDATE
) )
        UNION ALL
        SELECT
            1 AS "KEY",
...
--Receipt start
        ...
        WHERE
            ...
            AND TRUNC(araa.gl_date) BETWEEN ( TO_DATE(:p_from_date,'YYYY-MM-DD') ) AND NVL(TO_DATE(:p_to_date,'YYYY-MM-DD'),TRUNC(SYSDATE) )
        UNION ALL
        SELECT
            1 AS "KEY",
            'Receipt'--rec_line.transaction_type
...
--Adjustments start
        ...
        SELECT
           ...
           ,
            ...
            TO_CHAR(aaa.apply_date,'YYYY-MM-DD') trx_date --check if apply date is trx_date
           ,
            'RECEIVABLE' accounting_class_code,
        ...
        WHERE
            ...
            AND glp.period_set_name = 'CCL CALENDAR'
            AND glp.period_name NOT LIKE 'ADJ%'
            AND TRUNC(aaa.gl_date) BETWEEN ( TO_DATE(:p_from_date,'YYYY-MM-DD') ) AND NVL(TO_DATE(:p_to_date,'YYYY-MM-DD'),TRUNC(SYSDATE) )
        UNION ALL
        SELECT
            1 AS "KEY",
            'Adjustment' transaction_type,
