...
SELECT DISTINCT ROWNUM AS Seq_num1, b.*
FROM
(
SELECT DISTINCT a.* 
FROM
...
TO_CHAR(acra.receipt_date,'DD/MM/YY') 	AS tax_date1
,TO_CHAR(acra.doc_sequence_value) 		AS tax_invoice_number1
,TO_CHAR(rcta.trx_date,'DD/MM/YY') 		AS invoice_date1
,TO_CHAR(rcta.trx_number) 				AS invoice_number1
,rcta.exchange_rate 					AS exchange_rate1

,bill_to.party_name 					AS customer1
,NVL(
...
AND party_type_code = 'THIRD_PARTY')) AS customer_vat1
,(CASE 
...
AND enabled_flag = 'Y'
AND ROWNUM = 1
AND TRUNC (SYSDATE) BETWEEN NVL (start_date_active,TRUNC (SYSDATE))AND NVL (end_date_active,TRUNC (SYSDATE))) AS le_name1
...
AND enabled_flag = 'Y'
 AND ROWNUM = 1
 AND TRUNC (SYSDATE) BETWEEN NVL (start_date_active,TRUNC (SYSDATE))AND NVL (end_date_active,TRUNC (SYSDATE))) AS sender_address1,
NVL((SELECT description
 ...
 AND NVL (end_date_active,
  TRUNC (SYSDATE))),
xle.registration_number) AS vat_reg_no1
...
FROM RA_CUSTOMER_TRX_ALL ract2
WHERE ract2.previous_customer_trx_id =rcta.customer_trx_id) AS tax_date1

,
(SELECT ract2.trx_number
FROM RA_CUSTOMER_TRX_ALL ract2
WHERE ract2.previous_customer_trx_id =rcta.customer_trx_id) AS TAX_INVOICE_NUMBER1
,TO_CHAR(rcta.trx_date,'DD/MM/YY') 							AS invoice_date1
,TO_CHAR(rcta.trx_number) 									AS invoice_number1
,rcta.exchange_rate 										AS exchange_rate1
...
WHERE a.vat_rate1 ='0'
ORDER BY TO_DATE(a.tax_date1,'DD/MM/YY'), a.tax_invoice_number1)b
ORDER BY ROWNUM