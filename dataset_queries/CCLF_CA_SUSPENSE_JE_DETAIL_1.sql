SELECT        G.name                  ledger_name,
    ...
    WHERE     A.je_header_id          = B.je_header_id
      ...
      AND     F.segment2 = '1510198005'
      AND     g.ledger_category_code = 'PRIMARY'
      AND     G.name = :P_LEDGER
--	    AND     F.segment1 = 'GBN1'
--	    AND     b.period_name = 'MAY-16'
--      AND     C.user_je_source_name  = 'CCL_AP_INTERFACE'
/*      AND     ((fvv.attribute5 = 'USD' AND g.name LIKE '%FCY') OR  
               (fvv.attribute5 <> 'USD' AND g.name LIKE '%PRM'))      */
--      AND     (g.name LIKE '%FCY' OR g.name LIKE '%PRM')
      ...
      AND  ((C.user_je_source_name  = :P_JE_SOURCE AND :P_JE_SOURCE IS NOT NULL) OR             (1 = 1 AND :P_JE_SOURCE IS NULL))
