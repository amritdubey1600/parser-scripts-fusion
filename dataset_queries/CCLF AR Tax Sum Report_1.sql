/*  
*************************************************************************************************
-- Name             :  CCLF AR TAX Sum Report - Canada Specific
-- Date             :  
-- Author           :   
-- Type             : Sql  
*************************************************************************************************
-- Change history     
-- Version     REL                 			Date          Developer                       Description    
-- 1.0          --  						--			Initial version                   CCLF AR TAX Sum Report - Canada Specific
-- 1.1         REL-087                    04/03/24            Amjad 				      GERITM44801596:for GS25 there no Output for TAXAMT columns 
-- 1.2         REL-89                     14/06/24            Amjad                       GERITM45698076: for GS25 there no output for GST,HST and QST columns --#
*************************************************************************************************
*/
...
substr(RACTT.NAME,1,6)                       From_BUC,
...
decode(apba.instrument_type,'BANKACCOUNT',apba.account_number,'CREDITCARD',apba.card_number,'DEBITCARD',apba.card_number,null) CUSTOMER_BANK_NUM,
  ...
  To_Char(RACT.SHIP_DATE_ACTUAL,'YYYY-MM-DD')                   SHIP_DATE,
	...
	------------------added below code for REL-087 GERITM44801596---------------
	/* commented for REL-089  GERITM45698076
	CASE WHEN RACT.org_id IN (SELECT bu.bu_id 
	...
	THEN
	(
	select  round((sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2)
		from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb 
		where a.customer_trx_id= RACT.customer_trx_id 
		and  a.LINE_TYPE='TAX' 
		and a.vat_tax_id = zb.TAX_RATE_ID 
		 AND (UPPER (zb.tax_regime_code) LIKE '%GST%' -- REL-035 Added AND keyword
							OR UPPER (zb.tax_regime_code) LIKE '%HST%'
							OR UPPER (zb.tax_regime_code) LIKE '%AUSTRALIA%'
							)
	)
	else
	(select round((sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2) from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb where a.customer_trx_id= ract.CUSTOMER_TRX_ID and  a.LINE_TYPE='TAX' and /* a.Line_number=1 and */ --a.vat_tax_id = zb.TAX_RATE_ID  and zb.tax_regime_code like '%GST%'   ) 
   -- end taxamtGST,
   /* CASE WHEN RACT.org_id IN (SELECT bu.bu_id 
                                   FROM   FND_LOOKUP_VALUES_VL flvl, 
                                          FUN_ALL_BUSINESS_UNITS_V bu 
...
and  a.LINE_TYPE='TAX' 
and a.vat_tax_id = zb.TAX_RATE_ID 
 AND (UPPER (zb.tax_regime_code) LIKE '%PST%' -- REL-035 Added AND keyword
			        OR UPPER (zb.tax_regime_code) LIKE '%QST%'									
					OR UPPER (zb.TAX_RATE_CODE) LIKE '%PST%'
					OR UPPER (zb.TAX_RATE_CODE) LIKE '%QST%'
					)) 
	else
	(select round( ( sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2)  from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb  where a.customer_trx_id= ract.CUSTOMER_TRX_ID  and  a.LINE_TYPE='TAX' and /*a.Line_number=2 and */--a.vat_tax_id = zb.TAX_RATE_ID  and zb.tax_regime_code like '%PST%' ) 
    --end taxamtPST,
	-- end for REL-089 GERITM45698076
	
	----------end for REL-087 GERITM44801596-----------------------
--(select round((sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2) from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb where a.customer_trx_id= ract.CUSTOMER_TRX_ID and  a.LINE_TYPE='TAX' and /* a.Line_number=1 and */ a.vat_tax_id = zb.TAX_RATE_ID  and zb.tax_regime_code like '%GST%'   ) taxamtGST, -- comment for REL-087 GERITM44801596
(select round((sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2) from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb where a.customer_trx_id= ract.CUSTOMER_TRX_ID and  a.LINE_TYPE='TAX' and /* a.Line_number=1 and */ a.vat_tax_id = zb.TAX_RATE_ID  and (zb.tax_regime_code like '%GST%' OR UPPER (zb.TAX_RATE_CODE) LIKE '%GST%' )) taxamtGST, -- added for REL-089 GERITM45698076
(select round((sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2) from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb  where a.customer_trx_id= ract.CUSTOMER_TRX_ID  and  a.LINE_TYPE='TAX' and /* a.Line_number=1 and */ a.vat_tax_id = zb.TAX_RATE_ID  and (zb.tax_regime_code like '%HST%' OR UPPER (zb.TAX_RATE_CODE) LIKE '%HST%' )) taxamtHST, -- added TAX_RATE_CODE for REL-089 GERITM45698076 
(select round(( sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)) , 2) from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb  where a.customer_trx_id= ract.CUSTOMER_TRX_ID and  a.LINE_TYPE='TAX' and /*a.Line_number=2 and */a.vat_tax_id = zb.TAX_RATE_ID  and (zb.tax_regime_code like '%QST%' OR UPPER (zb.TAX_RATE_CODE) LIKE '%QST%' )) taxamtqST, --added TAX_RATE_CODE for REL-089 GERITM45698076 
--(select round( ( sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2)  from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb  where a.customer_trx_id= ract.CUSTOMER_TRX_ID  and  a.LINE_TYPE='TAX' and /*a.Line_number=2 and */a.vat_tax_id = zb.TAX_RATE_ID  and zb.tax_regime_code like '%PST%' ) taxamtPST , -- comment for REL-087 GERITM44801596  
(select round( ( sum(a.EXTENDED_AMOUNT) * nvl(RACT.EXCHANGE_RATE,1)),2)  from RA_CUSTOMER_TRX_LINES_ALL a, zx_rates_b zb  where a.customer_trx_id= ract.CUSTOMER_TRX_ID  and  a.LINE_TYPE='TAX' and /*a.Line_number=2 and */a.vat_tax_id = zb.TAX_RATE_ID  and (zb.tax_regime_code like '%PST%' OR UPPER (zb.TAX_RATE_CODE) LIKE '%PST%' )) taxamtPST , -- added for REL-089 GERITM45698076  
(SELECT round((SUM(extended_amount) * nvl(RACT.EXCHANGE_RATE,1)),2)
          FROM RA_CUSTOMER_TRX_LINES_ALL
         WHERE customer_trx_id = ract.customer_trx_id and line_type='LINE' ) InvoiceAmount ,
GLCC_RCVBL.segment1 
...
and GL_DIST_date.gl_posted_date is NOT NULL
