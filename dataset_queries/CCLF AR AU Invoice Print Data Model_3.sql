...
to_char((sum(c_line.extended_amount)+sum(c.extended_amount)),fnd_currency.get_format_mask(rct.invoice_currency_code,40)) total_f,
       ...
       c.customer_trx_id,
	    -- REL-030 added below code
	   rct.document_sub_type,
(SELECT meaning
              FROM fnd_lookup_values
              WHERE lookup_type = 'CIRRUSAR_SG_CUST_ACCOUNTING'
              AND lookup_code ='SG_CUST_NOTE_01'
AND LANGUAGE = 'US') tax_text_gst1,	   
(SELECT meaning
              FROM FND_LOOKUP_VALUES
              WHERE lookup_type = 'CIRRUSAR_SG_CUST_ACCOUNTING'
              and lookup_code ='SG_CUST_NOTE_02'
AND LANGUAGE = 'US') tax_text_gst2,	
 (SELECT meaning
              FROM FND_LOOKUP_VALUES
              WHERE lookup_type = 'CIRRUSAR_SG_CUST_ACCOUNTING'
              AND lookup_code ='SG_CUST_NOTE_03'
AND LANGUAGE = 'US') tax_text_gst3, 
(SELECT meaning
              FROM FND_LOOKUP_VALUES
              WHERE lookup_type = 'CIRRUSAR_SG_CUST_ACCOUNTING'
              and lookup_code ='SG_CUST_NOTE_04'
AND LANGUAGE = 'US') tax_text_gst4,	
(SELECT meaning
              FROM FND_LOOKUP_VALUES
              WHERE lookup_type = 'CIRRUSAR_SG_CUST_ACCOUNTING'
              and lookup_code ='SG_TAX_RATE_07'
AND LANGUAGE = 'US') tax_gst2,
TO_CHAR(SUM(NVL(rct.exchange_rate,1) * c_line.attribute10),FND_CURRENCY.GET_FORMAT_MASK(rct.invoice_currency_code,40)) tax_gst3
 -- REL-030 added above code
 -- REL-041 Added below code for Singapore by Sindhura Puppala
 ,c_line.tax_classification_code 
 -- REL-041 Added above code for Singapore by Sindhura Puppala
...
group by    
    ...
    z.tax_registration_number,
     c.customer_trx_id
    ,rct.document_sub_type  -- REL-030 Added
-- REL-041 Added below code for Singapore by Sindhura Puppala
	,c_line.tax_classification_code 
-- REL-041 Added above code for Singapore by Sindhura Puppala
