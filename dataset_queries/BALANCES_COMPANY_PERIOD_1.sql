SELECT
'0ledger_id~ledger_short_name~ledger_category_code~functional_currency_code~period_name~period_num~period_year~code_combination_id~company_code~
  ...
  begin_balance_dr_beq~begin_balance_cr_beq~period_net_dr~period_net_cr~period_net_dr_beq~period_net_cr_beq~account_type~last_update_date' FI_BALANCES  FROM DUAL
UNION  
SELECT ledger_id||'~'||ledger_short_name||'~'||ledger_category_code||'~'||functional_currency_code||'~'||period_name||'~'||period_num||'~'||period_year||'~'||code_combination_id||'~'||company_code||'~'||
  account||'~'||trading_partner||'~'||cost_center||'~'||geography||'~'||project_code||'~'||ref_code||'~'||product_line||'~'||book_type||'~'||future1||'~'||future2||'~'||BAL_CURRENCY_CODE||'~'||begin_balance||'~'||
  begin_func_equivalent||'~'||ptd_balance||'~'||ptd_func_equivalent||'~'||ytd_balance||'~'||ytd_func_equivalent||'~'||qtd_balance||'~'||qtd_func_equivalent||'~'||begin_balance_dr||'~'||begin_balance_cr||'~'||
  begin_balance_dr_beq||'~'||begin_balance_cr_beq||'~'||period_net_dr||'~'||period_net_cr||'~'||period_net_dr_beq||'~'||period_net_cr_beq||'~'||account_type||'~'||last_update_date  "FI_BALANCES"
FROM
  (SELECT TO_CHAR(l.ledger_id) ledger_id, l.name ledger_short_name, l.ledger_category_code, l.currency_code functional_currency_code, b.period_name, 
  ...
  AND  ((l.name LIKE '%FCY' AND ccv.functional_currency = SUBSTR(l.name,4,3)) OR 
