...
SELECT sender_id||','||data_type||','||period_name||','||company_code||','||account||','||trading_partner||','||cost_center||','||
 ...
 functional_currency_code||','||to_char(ytd_func_equivalent)||','||TO_CHAR(begin_func_equivalent)||','||
  ...
  TO_CHAR(rep_curr_ytd_balance)||','||TO_CHAR(rep_curr_begin_balance)||','||TO_CHAR(rep_curr_ptd_balance)||','||rep_ledg_short_name||','||pri_ledg_short_name DR4_TRANSP
FROM (
SELECT sender_id,data_type,PERIOD_NAME,COMPANY_CODE,ACCOUNT,TRADING_PARTNER,COST_CENTER,GEOGRAPHY,PROJECT_CODE,REFERENCE_CODE,PRODUCT_LINE,BOOK_TYPE,FUTURE1,FUTURE2,functional_currency_code,
    ...
    SUM(NVL(rep_curr_begin_balance,0)) rep_curr_begin_balance, SUM(NVL(rep_curr_ptd_balance,0)) rep_curr_ptd_balance,rep_ledg_short_name,pri_ledg_short_name	
FROM  (
-- UPDATED by Subba,Aseem on Gaurav request 07-JUL-2016
/* SELECT 'EMTRCA' sender_id,'CCLBALEXFCES' data_type, glb.period_name ,glc.segment1 company_code,glc.segment2 account,glc.segment3 trading_partner,glc.segment4 cost_center,glc.segment5 geography,
        ...
        (SELECT  led_rep.currency_code
  	   ...
  	   AND upper(led_rep.description)   LIKE '%FCY%'		
...
AND ccv.company_code IN('T283', 'T117', 'T502')
...
SELECT CCLFUS_DR4_TRANS .*,'P' key
FROM CCLFUS_DR4_TRANS 
UNION
SELECT 'EMTRCA,CCLBALEXFCES,'||TO_CHAR(SYSDATE,'YYYYMMDD')||',TRAILER,'||TO_CHAR(COUNT(*)) ||',P' DR4_TRANSP ,'P' key  FROM CCLFUS_DR4_TRANS
ORDER BY 1 DESC